...
<head>
    ...
    <title>@yield('title', 'Area - Iseki Part NG')</title>
    <meta name="description" content="Area Panel - Iseki Part NG Management System">
    ...
    <style>
        ...
        *{font-family:'Inter',-apple-system,BlinkMacSystemFont,sans-serif}
        ...
        .hero-banner{background:linear-gradient(135deg,var(--pink-500) 0%,var(--pink-700) 100%);color:#fff;padding:2rem 1.5rem;text-align:left;margin-bottom:2rem;position:relative;overflow:hidden;border-radius:20px;box-shadow:0 10px 30px rgba(162,28,175,.2)}
        .hero-banner h1{font-weight:800;font-size:1.8rem;letter-spacing:-.025em;position:relative;z-index:1}
        .hero-banner p{font-weight:400;opacity:.9;position:relative;z-index:1}
        .glass-card{background:var(--glass-bg);backdrop-filter:blur(20px);border:1px solid var(--glass-border);border-radius:16px;box-shadow:0 8px 32px rgba(219,39,119,.08);transition:all .3s}
        .glass-card:hover{box-shadow:0 12px 40px rgba(219,39,119,.15);transform:translateY(-2px)}
        ...
        .card-header-pink{background:linear-gradient(135deg,var(--pink-500),var(--pink-600));color:#fff;font-weight:700;border-radius:16px 16px 0 0!important;padding:1rem 1.25rem;border:none}
...
<body>
    ...
    <aside class="app-sidebar">
        <div class="app-sidebar-brand d-flex align-items-center">
            ...
            <div class="brand-text">Area Panel<span class="d-block brand-sub">Iseki Part NG</span></div>
        </div>
        <nav class="app-sidebar-nav">
            <div class="nav-section">Menu</div>
            <a class="nav-link {{ request()->routeIs('area.dashboard') ? 'active' : '' }}" href="{{ route('area.dashboard') }}">
                <i class="bi bi-speedometer2"></i><span>Dashboard</span>
            </a>
            <a class="nav-link {{ request()->routeIs('area.create') ? 'active' : '' }}" href="{{ route('area.create') }}">
        ...
        <div class="app-sidebar-footer">
            <div class="d-flex align-items-center gap-2 mb-2">
                ...
                <div class="flex-grow-1">
                    <div class="text-white small fw-semibold">{{ auth()->user()->Name_User ?? 'Area' }}</div>
                    <div class="text-white-50" style="font-size:.65rem">Area</div>
                </div>
            </div>
