...
<div class="container pb-5">
    <div class="mb-3">
        <a href="{{ route('leader.dashboard') }}" class="btn btn-pink-outline">
    ...
    <form id="partNgForm" enctype="multipart/form-data">
        @csrf
        {{-- ===================== STEP 1: INPUT PART MANUAL ===================== --}}
        <div class="glass-card mb-4 fade-in" id="step1">
            <div class="card-header-pink">
                <i class="bi bi-1-circle-fill me-2"></i>Step 1: Input Part NG Manual
            </div>
            <div class="card-body p-4">
                ...
                <div class="row text-start justify-content-center mt-3">
                    ...
                    <div class="col-12 col-md-6 mb-3">
                        ...
                        <input type="text" id="name_item_rack" name="Name_Item_Rack" class="form-control form-control-lg fw-bold" style="color: var(--pink-600);" placeholder="Ketik Nama Part" required>
                    </div>
                </div>

                <input type="hidden" id="Id_Rack" name="Id_Rack" value="">
                <input type="hidden" id="code_rack" name="Code_Rack" value="">

                <button type="button" class="btn btn-pink btn-lg w-100 mt-2" id="btnNext1">
        ...
        <div class="glass-card mb-4 d-none slide-up" id="step2">
            ...
            <div class="card-body p-4">
                ...
                <!-- Divisi (hidden if area autofills) -->
                @if(!$area)
                ...
                <!-- Proses (hidden if area autofills) -->
                @if(!$area)
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('btnNext1').addEventListener('click', () => {
            const codePart = document.getElementById('code_item_rack').value.trim();
            const namePart = document.getElementById('name_item_rack').value.trim();
        ...
        function updateCategoryHint() {
            ...
            const div = document.getElementById('divisi');
            const divVal = div ? div.value : '{{ $area->Divisi ?? "" }}';
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            document.querySelector('[data-remove-photo="' + i + '"]').addEventListener('click', function() {
                input.value = '';
                preview.src = '';
        ...
        document.getElementById('btnSubmit').addEventListener('click', function() {
            const desc = document.getElementById('desc_part_ng').value;
            const cat = document.getElementById('category_part_ng').value;
            ...
            if(!proses) return showNotification('Proses belum dipilih. Silakan pilih proses terlebih dahulu.', 'warning');
            if(!div) return showNotification('Divisi belum dipilih. Silakan pilih divisi terlebih dahulu.', 'warning');
            @endif
            ...
            if (cat == 'bukan tanggung jawab') {
                finalCategory = 'bukan tanggung jawab ' + '{{ $area->Divisi }}';
            }
            @else
            if (cat == 'bukan tanggung jawab') {
                finalCategory = 'bukan tanggung jawab ' + document.getElementById('divisi').value;
            }
            @endif
            ...
            @if($area)
            formData.set('Divisi', '{{ $area->Divisi }}');
            formData.set('proses', '{{ $area->Proses }}');
            @endif

            this.disabled = true;
            this.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Menyimpan...';

            fetch("{{ route('leader.store') }}", {
                method: 'POST',
                body: formData,
