...
<div class="container-fluid px-lg-5 py-4 fade-in">
    <div class="d-flex justify-content-between align-items-end mb-4 flex-wrap gap-3 border-bottom pb-3">
        <div>
            <h3 class="fw-800 mb-1" style="color:var(--ig800)">
                <i class="bi bi-check-circle-fill me-2 opacity-75"></i>Sudah Diproses — Bulan Ini
            </h3>
            <p class="text-muted mb-0">Data Part NG yang sudah diproses pada bulan {{ now()->translatedFormat('F Y') }}</p>
        </div>
        <div class="d-flex gap-2">
            ...
            <a href="{{ route('leader.report.unprocessed.monthly') }}" class="btn btn-outline-warning shadow-sm rounded-pill px-4 py-2 fw-bold">
                <i class="bi bi-hourglass me-2"></i>Belum Diproses (Bulan Ini)
            </a>
            <a href="{{ route('leader.export', array_merge(request()->all(), ['status' => 'processed'])) }}" class="btn btn-success shadow-sm rounded-pill px-4 py-2 fw-bold">
    ...
    <div class="glass-card p-3 border-0 mb-4">
        <form action="{{ route('leader.report.processed.monthly') }}" method="GET" class="row g-2 align-items-end">
            <div class="col-auto">
            ...
            <div class="col-auto">
                ...
                <a href="{{ route('leader.report.processed.monthly') }}" class="btn btn-light shadow-sm" style="border-radius:10px;font-size:.85rem;padding:.45rem 1.2rem;color:var(--ig700);font-weight:600">Reset</a>
            </div>
    ...
    <div class="card glass-card border-0" style="border-radius:16px;overflow:hidden;box-shadow:0 10px 40px rgba(99,102,241,.08)">
        <div class="card-header card-header-pink d-flex justify-content-between align-items-center py-3">
            <h6 class="mb-0 fw-bold fs-6"><i class="bi bi-table me-2"></i>Daftar Part NG Sudah Diproses — Bulan Ini</h6>
        </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="12" class="text-center py-5">
                                ...
                                <h6 class="fw-bold text-muted">Belum ada data yang diproses bulan ini</h6>
                                <p class="small text-muted">Silakan proses data terlebih dahulu.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="modal fade" id="processModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 glass-card">
                <div class="modal-header border-bottom-0 pb-0">
                    <h5 class="modal-title fw-bold" style="color:var(--ig800)"><i class="bi bi-gear-fill me-2"></i>Edit Proses Part NG</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
