...
@section('content')
    <div class="container-fluid px-lg-5 py-4 fade-in">
        <div class="d-flex justify-content-between align-items-end mb-4 flex-wrap gap-3 border-bottom pb-3">
            <div>
                <h3 class="fw-800 mb-1" style="color:var(--ig800)">
                    <i class="bi bi-clipboard2-data-fill me-2 opacity-75"></i>Dashboard Part NG
                </h3>
                <p class="text-muted mb-0">Statistik Part NG — {{ $monthLabel }}</p>
            </div>
            <div class="d-flex align-items-center gap-2">
                <a href="{{ request()->fullUrlWithQuery(['month' => $prevMonth]) }}" class="btn btn-sm" style="border-radius:10px;padding:.5rem .7rem;background:var(--ig100);color:var(--ig700);border:1px solid var(--ig300);font-weight:600">
            ...
            <a href="{{ request()->fullUrlWithQuery(['month' => $nextMonth]) }}" class="btn btn-sm" style="border-radius:10px;padding:.5rem .7rem;background:var(--ig100);color:var(--ig700);border:1px solid var(--ig300);font-weight:600">
                <i class="bi bi-chevron-right"></i>
            </a>
            <a href="{{ route('leader.create') }}" class="btn" style="border-radius:10px;background:linear-gradient(135deg,var(--pink-500),var(--pink-600));color:#fff;font-weight:600;border:none;padding:.5rem 1rem;box-shadow:0 4px 14px rgba(236,72,153,.3)">
        ...
        <div class="row g-4">
        <div class="col-lg-8">
            <div class="card glass-card border-0 mb-4" style="border-radius:16px;overflow:hidden;box-shadow:0 10px 40px rgba(99,102,241,.08)">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        createChart('ngChart', {!! $chartDates->toJson() !!}, {!! $chartTotals->toJson() !!}, '#4f46e5', 'rgba(79, 70, 229, ');
