...
<div class="container-fluid px-lg-5 py-4 fade-in">
    <div class="d-flex justify-content-between align-items-end mb-4 flex-wrap gap-3 border-bottom pb-3">
        <div>
            ...
            <p class="text-muted mb-0">Data Part NG yang belum diproses pada bulan {{ now()->translatedFormat('F Y') }}</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('leader.report') }}" class="btn btn-outline-secondary shadow-sm rounded-pill px-4 py-2 fw-bold">
                <i class="bi bi-list-ul me-2"></i>Semua Report
            </a>
            <a href="{{ route('leader.report.processed.monthly') }}" class="btn btn-outline-success shadow-sm rounded-pill px-4 py-2 fw-bold">
                <i class="bi bi-check-circle me-2"></i>Sudah Diproses (Bulan Ini)
            </a>
            <a href="{{ route('leader.export', array_merge(request()->all(), ['status' => 'unprocessed'])) }}" class="btn btn-success shadow-sm rounded-pill px-4 py-2 fw-bold">
    ...
    <div class="glass-card p-3 border-0 mb-4">
        <form action="{{ route('leader.report.unprocessed.monthly') }}" method="GET" class="row g-2 align-items-end">
            <div class="col-auto">
                <label class="filter-label mb-1"><i class="bi bi-calendar me-1"></i>Bulan</label>
                <input type="month" name="month" class="form-control bg-light border-0 shadow-sm" style="border-radius:10px;font-size:.85rem" value="{{ request('month', now()->format('Y-m')) }}">
            </div>
            <div class="col-auto">
            ...
            <div class="col-auto">
                ...
                <a href="{{ route('leader.report.unprocessed.monthly') }}" class="btn btn-light shadow-sm" style="border-radius:10px;font-size:.85rem;padding:.45rem 1.2rem;color:var(--ig700);font-weight:600">Reset</a>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-1 flex-wrap" style="max-width:110px;margin:0 auto">
                                    ...
                                    <img src="{{ asset($photo) }}" class="img-thumbnail rounded-3 shadow-sm border-0" style="height:45px;width:45px;object-fit:cover;cursor:pointer" onclick="showPhoto('{{ asset($photo) }}')" alt="Foto">
                                    @empty
                        ...
                        <tr>
                            <td colspan="12" class="text-center py-5">
                                <div class="opacity-50 mb-3" style="color:var(--ig400)"><i class="bi bi-check2-circle fs-1"></i></div>
                                <h6 class="fw-bold text-muted">Semua data bulan ini sudah diproses</h6>
                                <p class="small text-muted">Tidak ada data Part NG yang belum diproses untuk bulan ini.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
