...
@section('content')
    <div class="container-fluid px-lg-5 py-4 fade-in">
        <div class="d-flex justify-content-between align-items-end mb-4 flex-wrap gap-3 border-bottom pb-3 border-pink-200">
            <div>
                <h3 class="fw-800 mb-1" style="color:var(--pink-800)">
                    <i class="bi bi-hourglass-split me-2 opacity-75"></i>Belum Diproses — Bulan Ini
                </h3>
                <p class="text-muted mb-0">Data Part NG yang belum diproses pada bulan {{ now()->translatedFormat('F Y') }}
                </p>
            </div>
            <div class="d-flex gap-2">
                <a href="{{ route('admin.report') }}"
                    class="btn btn-outline-secondary shadow-sm rounded-pill px-4 py-2 fw-bold">
                    <i class="bi bi-list-ul me-2"></i>Semua Report
                </a>
                <a href="{{ route('admin.report-processed-monthly') }}"
                    class="btn btn-outline-success shadow-sm rounded-pill px-4 py-2 fw-bold">
                    <i class="bi bi-check-circle me-2"></i>Sudah Diproses (Bulan Ini)
                </a>
                <button type="button" class="btn btn-success shadow-sm rounded-pill px-4 py-2 fw-bold"
        ...
        <div class="glass-card p-3 border-0 mb-4">
            <form action="{{ route('admin.report-unprocessed-monthly') }}" method="GET" class="row g-2 align-items-end">
                <div class="col-auto">
                    <label class="filter-label mb-1"><i class="bi bi-calendar me-1"></i>Bulan</label>
                    <input type="month" name="month" class="form-control bg-light border-0 shadow-sm"
                        style="border-radius:10px;font-size:.85rem" value="{{ request('month', now()->format('Y-m')) }}">
                </div>
                <div class="col-auto">
                    <label class="filter-label mb-1"><i class="bi bi-calendar-week me-1"></i>Minggu</label>
                    <select name="week" class="form-select bg-light border-0 shadow-sm"
                ...
                <div class="col-auto">
                    ...
                    <select name="divisi" class="form-select bg-light border-0 shadow-sm"
                        style="border-radius:10px;font-size:.85rem">
                        <option value="">All</option>
                        <option value="Assembling" {{ request('divisi') == 'Assembling' ? 'selected' : '' }}>Assembling
                        </option>
                ...
                <div class="col-auto">
                    ...
                    <select name="category" class="form-select bg-light border-0 shadow-sm"
                        style="border-radius:10px;font-size:.85rem">
                        <option value="">All</option>
                        <option value="part scrap" {{ request('category') == 'part scrap' ? 'selected' : '' }}>Part scrap
                        </option>
                ...
                <div class="col-auto">
                    <button type="submit" class="btn btn-pink shadow-sm"
                        style="border-radius:10px;font-size:.85rem;padding:.45rem 1.2rem">
                        <i class="bi bi-search me-1"></i>Filter
                    </button>
                    <a href="{{ route('admin.report-unprocessed-monthly') }}" class="btn btn-light shadow-sm"
                        style="border-radius:10px;font-size:.85rem;padding:.45rem 1.2rem;color:var(--pink-700);font-weight:600">Reset</a>
                </div>
        ...
        <div class="card glass-card border-0 custom-table-container">
            <div class="card-header card-header-pink d-flex justify-content-between align-items-center py-3">
                <h6 class="mb-0 fw-bold fs-6"><i class="bi bi-table me-2"></i>Daftar Part NG Belum Diproses — Bulan Ini</h6>
            </div>
                        ...
                        <tbody>
                            @forelse($parts as $index => $p)
                                <tr>
                                    ...
                                    <td>
                                        <div class="fw-bold fs-6" style="color:var(--pink-800)">
                                            {{ \Carbon\Carbon::parse($p->Date_Part_Ng)->format('d M Y') }}</div>
                                        <div class="small text-muted fw-500">
                                            {{ \Carbon\Carbon::parse($p->Date_Part_Ng)->format('H:i') }}</div>
                                    </td>
                                    <td>
                                        <div class="badge-pink shadow-sm mb-1">{{ $p->Code_Rack }}</div>
                                        <div class="fw-bold small text-dark mb-1">{{ $p->Name_Item_Rack }}</div>
                                        <div class="text-muted small font-monospace bg-light rounded px-2 py-1 d-inline-block">
                                            {{ $p->Code_Item_Rack }}</div>
                                    </td>
                                    <td>
                                        <p class="mb-0 small"
                                            style="max-width:200px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden"
                                            title="{{ $p->Desc_Part_Ng }}">{{ $p->Desc_Part_Ng }}</p>
                                    </td>
                                    <td>
                                        <div><span class="badge bg-info">{{ $p->Divisi ?? '-' }}</span></div>
                                        <div class="mt-1"><span class="badge"
                                                style="background:var(--pink-100);color:var(--pink-800);border:1px solid var(--pink-200);white-space:normal;text-align:left;line-height:1.2">{{ $p->Category_Part_Ng }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        @if($p->penanggungjawab)
                                            <span class="small fw-bold text-success"><i
                                                    class="bi bi-person-check-fill me-1"></i>{{ $p->penanggungjawab }}</span>
                                        @else
                                    ...
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-pink fw-bold w-100"
                            ...
                            @empty
                                <tr>
                                    <td colspan="12" class="text-center py-5">
                                        <div class="opacity-50 mb-3" style="color:var(--pink-400)"><i
                                                class="bi bi-check2-circle fs-1"></i></div>
                                        <h6 class="fw-bold text-muted">Semua data bulan ini sudah diproses</h6>
                                        <p class="small text-muted">Tidak ada data Part NG yang belum diproses untuk bulan ini.
                                        </p>
                                    </td>
                                </tr>
                            @endforelse
                ...
                <div class="modal-content border-0 glass-card">
                    <div class="modal-header border-bottom-0 pb-0">
                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                            style="background-color:var(--pink-100);border-radius:50%;opacity:1;padding:.8rem;margin:.5rem"></button>
                    </div>
                    <div class="modal-body text-center pt-0 pb-4">
                        <img id="modalPhotoSrc" src="" class="img-fluid rounded-4 shadow-lg" alt="Foto Detail"
                            style="max-height:80vh;border:4px solid white">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="processModal" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content border-0 glass-card">
                    <div class="modal-header border-bottom-0 pb-0">
                        <h5 class="modal-title fw-bold" style="color:var(--pink-800)"><i
                                class="bi bi-gear-fill me-2"></i>Proses Part NG</h5>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex gap-3 mb-2">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="pj_type" id="pjMember"
                                            value="member" checked>
                                        <label class="form-check-label small" for="pjMember">Pilih Member</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="pj_type" id="pjManual"
                                            value="manual">
                                        <label class="form-check-label small" for="pjManual">Lain-lain</label>
                                    </div>
                                </div>
                                <div id="pjMemberGroup">
                                    <input type="text" id="memberSearch" class="form-control" style="border-radius:12px"
                                        placeholder="Cari nama member...">
                                    <div id="memberResults" class="list-group mt-1"
                                        style="max-height:200px;overflow-y:auto;display:none;position:absolute;z-index:1000;width:calc(100% - 2rem);box-shadow:0 4px 12px rgba(0,0,0,.1);border-radius:8px">
                                    </div>
                                ...
                                <div id="pjManualGroup" class="d-none">
                                    <input type="text" id="penanggungjawabManual" class="form-control"
                                        style="border-radius:12px" placeholder="Ketik keterangan lain...">
                                </div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-bold small text-muted text-uppercase">Penyebab</label>
                                <textarea name="penyebab" class="form-control" rows="3" style="border-radius:12px" required
                                    placeholder="Masukkan penyebab"></textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-bold small text-muted text-uppercase">Pencegahan /
                                    Penanganan</label>
                                <textarea name="penanganan" class="form-control" rows="3" style="border-radius:12px"
                                    required placeholder="Masukkan pencegahan/penanganan"></textarea>
                            </div>
                        </div>
                        <div class="modal-footer border-top-0 pt-0">
                            <div class="d-flex justify-content-between align-items-center w-100">
                                <button type="button" class="btn btn-outline-secondary" id="prevBtn" onclick="prevPart()"
                                    style="display:none">
        ...
        <div class="modal fade" id="exportModal" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 glass-card">
                    <div class="modal-header border-bottom-0 pb-0">
                        <h5 class="modal-title fw-bold" style="color:var(--pink-800)"><i
                                class="bi bi-file-earmark-excel me-2"></i>Export Excel</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <form id="exportForm" action="{{ route('admin.export') }}" method="GET">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="month" name="month" class="form-control" style="border-radius:12px"
